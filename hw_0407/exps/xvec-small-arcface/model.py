...
class tdnn(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.softmax = self.add_weight(shape=(64, self.vocab),
      trainable=True, name='softmax')
  ...
  def call(self, inputs, training=None):
    ...
    if ref is not None:
      x_norm = tf.math.l2_normalize(x, -1)
      w_norm = tf.math.l2_normalize(self.softmax, 0)

      xw = tf.linalg.matmul(x_norm, w_norm)
      m_mask = tf.one_hot(ref, self.vocab, on_value=self.margin, dtype=tf.float32)
      
      x = tf.math.cos(tf.math.acos(xw) + m_mask)
      x *= self.scale
