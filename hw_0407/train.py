...
parser.add_argument("--eval-step", type=int, required=False, default=10) 
parser.add_argument("--train-step", type=int, required=False, default=1000) 
...
def train_step(pcm, ref):
  with tf.GradientTape() as tape:
    loss = m((model.mel_filterbank(pcm), ref), training=True)
...
for idx, data in enumerate(dataset):
  ...
  if idx > 0 and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] lr[{:.2e}]".format(idx, loss, lr))
