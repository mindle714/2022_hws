...
class EncoderCell(nn.Module):
    ...
    def forward(self, input, hidden1, hidden2, hidden3):
        ...
        hidden1 = self.rnn1(x, [hidden1, hidden1])
        x = hidden1[0]

        hidden2 = self.rnn2(x, [hidden2, hidden2])
        x = hidden2[0]

        hidden3 = self.rnn3(x, [hidden3, hidden3])
        x = hidden3[0]
...
class DecoderCell(nn.Module):
    ...
    def forward(self, input, hidden1, hidden2, hidden3, hidden4):
        ...
        hidden4 = self.rnn4(x, [hidden4, hidden4])
        x = hidden4[0]
        x = F.pixel_shuffle(x, 2)

        x = F.tanh(self.conv2(x)) / 2
        return x, hidden1[0], hidden2[0], hidden3[0], hidden4[0]
